...
module.exports = ({year, month, day}, isYear = false) => {

  const MONTH_LENGTH = monthData(year, month).length;
  const START_OF_MONTH = zeller({year, month, day: 1});

  let formattedMonthString = printMonthHeader(year, month, isYear) + printDaysHeader();
